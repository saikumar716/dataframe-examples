...
object Rdd2DfThruExplicitSchema {
  def main(args: Array[String]): Unit = {
    ...
    txnFctDf = txnFctDf
      ...
      .withColumn("Ater_Increament", txnFctDf("amount")+2)
      .withColumn("Increament", txnFctDf("amount") - txnFctDf("amount")+2)
    txnFctDf.printSchema()
    txnFctDf.show(5, false)
    ...
    var txnAggDf = txnFctDf
      ...
      .groupBy("merchant_id")
      .agg("amount" -> "sum", "status" -> "approx_count_distinct", "Increament" -> "sum")
    ...
    txnAggDf = txnAggDf
      ...
      .withColumnRenamed("sum(Increament)", "Increament")
    txnAggDf.show(5, false)
