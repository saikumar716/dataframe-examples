...
//    employeeDf.join(empRoleDf, $"id" === $"id").show(false)   // Ambiguous column name "id"
    ...
    employeeDf.join(empRoleDf, Seq("id"), "leftsemi").show(false)
   val left = employeeDf.join(empRoleDf, Seq("id"), "left")
    left.show(false)
    val left_outer = employeeDf.join(empRoleDf, Seq("id"), "left_outer")
    left_outer.show(false)
    left.except(left_outer).show()
